...
class TLM_Settings {
	...
	public static function get_default_settings() {
		return array(
			...
			'menu_title'       => __( 'Destinations', 'tour-location-manager' ),
			'show_counts'      => 1,
			'expand_all'       => 0,
			'breadcrumb_label' => __( 'Browse by Destination', 'tour-location-manager' ),
		);
	}
	...
	public function add_settings_page() {
		add_submenu_page(
			...
			__( 'Location Manager Settings', 'tour-location-manager' ),
			__( 'Location Settings', 'tour-location-manager' ),
			'manage_woocommerce',
			'tlm-settings',
	...
	public function register_settings() {
		...
		add_settings_section(
			'tlm_general_section',
			__( 'General Settings', 'tour-location-manager' ),
			'__return_false',
			'tlm-settings'
		);
		...
		add_settings_field(
			'taxonomy_slug',
			__( 'Taxonomy Slug', 'tour-location-manager' ),
		...
		add_settings_field(
			'archive_base',
			__( 'Archive URL Base', 'tour-location-manager' ),
		...
		add_settings_field(
			'menu_title',
			__( 'Frontend Menu Title', 'tour-location-manager' ),
		...
		add_settings_field(
			'breadcrumb_label',
			__( 'Tree Heading Label', 'tour-location-manager' ),
		...
		add_settings_field(
			'show_counts',
			__( 'Show Product Counts', 'tour-location-manager' ),
		...
		add_settings_field(
			'expand_all',
			__( 'Expand Tree by Default', 'tour-location-manager' ),
	...
	public function sanitize_settings( $input ) {
		...
		// Flush rewrite rules if slug or base changed, since permalinks depend on them.
		flush_rewrite_rules();
	...
	public function field_taxonomy_slug() {
		...
		printf(
			...
			esc_html__( 'Internal taxonomy slug (advanced — changing this requires re-saving permalinks).', 'tour-location-manager' )
		);
	}
	...
	public function field_archive_base() {
		...
		printf(
			...
			esc_html__( 'Example: example.com/location/usa/california/los-angeles/', 'tour-location-manager' )
		);
	}
	...
	public function field_show_counts() {
		...
		printf(
			...
			esc_html__( 'Display the number of tours/services next to each location.', 'tour-location-manager' )
		);
	}
	...
	public function field_expand_all() {
		...
		printf(
			...
			esc_html__( 'Render the full tree expanded (otherwise it expands via JavaScript on click).', 'tour-location-manager' )
		);
	}
	...
	public function render_settings_page() {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Tour Location Manager Settings', 'tour-location-manager' ); ?></h1>
			...
			<h2><?php esc_html_e( 'Shortcode Usage', 'tour-location-manager' ); ?></h2>
			<p>
				<?php
				printf(
					/* translators: %s: shortcode tag */
					esc_html__( 'Use %s anywhere (pages, posts, widgets) to display the full location tree.', 'tour-location-manager' ),
					'<code>[tour_location_menu]</code>'
				);
				?>
			</p>
			<p><?php esc_html_e( 'Optional attributes:', 'tour-location-manager' ); ?></p>
			<ul>
				<li><code>parent="0"</code> — <?php esc_html_e( 'Start the tree from a specific term ID (0 = top level / countries).', 'tour-location-manager' ); ?></li>
				<li><code>depth="3"</code> — <?php esc_html_e( 'Maximum levels to render (1-3).', 'tour-location-manager' ); ?></li>
				<li><code>show_counts="yes"</code> — <?php esc_html_e( 'Override the global "show counts" setting.', 'tour-location-manager' ); ?></li>
			</ul>
		</div>
		<?php
	}
